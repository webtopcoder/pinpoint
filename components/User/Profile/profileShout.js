...
import { Image as Antimage, Button, Avatar, List, Space, Skeleton, Typography } from 'antd';
                                            ...
                                            <List
                                                ...
                                                renderItem={(item, index) => (
                                                    <List.Item
                                                        key={index}
                                                        actions={[
                                                            <IconText postID={item._id} text={item?.like ? (item.like).length : 0} key="list-vertical-like-o" />,
                                                        ]}
                                                    >
                                                                ...
                                                                (<div className="custom-list-content" style={{
                                                                    ...
                                                                    <Antimage.PreviewGroup>
                                                                        {item.image.map((item1, index) =>
                                                                            <Antimage loader={myLoader} width={'25%'} src={imgurl + '/' + item1} />
                                                                        )}
                                                                    </Antimage.PreviewGroup>
                                                                </div>) : ''}
                                                        </Skeleton>
...
const mapDispatchToProps = dispatch => ({
    ...
    ongetShoutout: (data, count, search, cb) => dispatch(getShoutout(data, count, search, cb)),
})
