        ...
        <div className="thread-sender">
          ...
          <div className="thread-from">
            <div className="from">
              <Tooltip title="View Profile" color={"blue"}>
                <a
                  onClick={() =>
                    window.open(
                      baseUrl + "/user/" + record.from_user.id + "/activity",
                      "_blank"
                    )
                  }
                >
            ...
            <span className="activity">
              last received:{" "}
              {new Date(record.inbox[0].createdAt).toLocaleDateString(
                undefined,
                {
                  year: "numeric",
                  month: "long",
          ...
          <p>
            <Tooltip title="View Message" color={"blue"}>
              <a onClick={() => selectedInboxinfo(record)}>
                {record.inbox[0].subject.length > 30
                  ? record.inbox[0].subject.substring(0, 30) + "..."
                  : record.inbox[0].subject}
              </a>
            </Tooltip>
          </p>
        </div>
        ...
        <Space size="middle">
          {!record.is_read ? (
            <Tooltip title="Mark as Read" color={"blue"}>
              <a
                onClick={() =>
                  mark_Inbox(record.inbox[0].from, !record.is_read)
                }
                className="mark-read"
              >
          ...
          <Tooltip title="Are you sure?" color={"blue"}>
            <a
              onClick={() => delete_Inbox(record.inbox[0].from)}
  ...
  return (
    <>
      <Row className="mail-inbox">
        <Col md={24} sm={24} xs={24} key="vertical">
          <Table
            columns={columnes}
            rowSelection={{
              type: selectionType,
